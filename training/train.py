...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Path to the root directory where the best model is saved",
)
parser.add_argument(
    "--output_csv", type=str, required=True, help="Path to output CSV file for results"
)
parser.add_argument(
    "--num_classes", type=int, default=24, help="Number of classes for segmentation"
)
parser.add_argument(
    "--max_iterations",
    type=int,
...
parser.add_argument(
    ...
    type=int,
    default=500,
    help="Number of validation images for evaluation",
)
parser.add_argument(
    "--num_train_images", type=int, default=40, help="Number of training images to use"
)
parser.add_argument(
    "--patch_size", type=int, default=128, help="Size of patches for training"
)
parser.add_argument(
    "--spatial_dims",
    type=int,
    default=3,
    help="For 3D data it is 3 for 2D data it is 2",
)
parser.add_argument(
    "--feature_size", type=int, default=96, help="Initial Filters for SegResNet Model"
)
parser.add_argument(
    "--num_samples", type=int, default=4, help="Number of Samples per batch"
)
...
def main(args):
    ...
    # Setup Data Loaders
    num_cpus = torch.get_num_threads() - 16
    ...
    train_loader = DataLoader(
        train_ds, batch_size=1, shuffle=True, num_workers=num_cpus, pin_memory=True
    )
    val_loader = DataLoader(
        val_ds, batch_size=1, shuffle=False, num_workers=num_cpus, pin_memory=True
    )
    print("Dataset is loaded and prepared for training and validation...")
    print(
        f"Training Dataset: {len(train_loader)} | Validation Dataset: {len(val_loader)}"
    )
    ###################### DEFINING THE MODEL ################################################
    ...
    while global_step < args.max_iterations:
        global_step, dice_val_best, global_step_best = trainer.train(
            global_step, train_loader, val_loader, dice_val_best, global_step_best
        )
    print(
        f"Global Step: {global_step} | Best Dice: {dice_val_best} | Global Best: {global_step_best}"
    )
